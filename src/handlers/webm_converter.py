...
def handle(bot, update):
    ...
    logger.info("webm: Handling webm request from user '%s' in group '%s'",
    ...
    if document.file_name.endswith(".webm"):
        ...
        try:
            ...
            logger.info("webm: Downloaded webm - " +
                        str(document.file_id) + '.webm')

            stream = ffmpeg.input(
                'resources/' + str(document.file_id) + '.webm')
            ...
            if has_audio:
                ...
                output = ffmpeg.output(joined[0], joined[1], 'resources/' + str(
                    document.file_id) + '.mp4', vcodec='libx265', crf=28, acodec='libvorbis')
                output = ffmpeg.overwrite_output(output)
                ffmpeg.run(output, capture_stdout=True, capture_stderr=True)
            else:
                output = ffmpeg.output(video, 'resources/' + str(document.file_id) +
                                       '.mp4', vcodec='libx265', crf=28, acodec='libvorbis')
                output = ffmpeg.overwrite_output(output)
                ffmpeg.run(output)
            ...
            pretty_time_delta = chaddi_util.pretty_time_delta(
                difference.seconds)
            logger.info("webm: Finished converting webm=%s , timeTaken=%s", str(
                document.file_id), pretty_time_delta)
            ...
            caption = og_sender + " converted your WebM to MP4 in " + \
                str(pretty_time_delta) + " \n (＾－＾)"

            bot.send_video(
                chat_id=update.message.chat_id,
                video=open('resources/' +
                           str(document.file_id) + '.mp4', 'rb'),
                timeout=5000,
                caption=caption
            )
            ...
            response = "Error occured during WebM conversion - `" + \
                str(e) + "`"
