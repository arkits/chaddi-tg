...
def reschedule_saved_jobs(job_queue):
    ...
    for sj in sjs:
        ...
        job_name = build_job_name(sj.job_context["chat_id"], sj.job_context["from_bakchod_id"], sj.job_context["job_id"])

        job_queue.run_once(
            reminder_handler, due_seconds, context=json.dumps(sj.job_context), name=job_name
        )
